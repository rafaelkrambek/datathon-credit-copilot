...
def get_applicant(sk_id: int) -> dict | None:
    ...
    return {
        ...
        "years_employed": round(-r["DAYS_EMPLOYED"] / 365, 1)
        if r["DAYS_EMPLOYED"] != 365243
        else None,
...
def get_bureau_history(sk_id: int) -> dict:
    ...
    return {
        ...
        "total_debt": float(
            rows["AMT_CREDIT_SUM_DEBT"].sum() if "AMT_CREDIT_SUM_DEBT" in rows else 0
        ),
        "total_overdue": float(
            rows["AMT_CREDIT_SUM_OVERDUE"].sum() if "AMT_CREDIT_SUM_OVERDUE" in rows else 0
        ),
...
def get_internal_history(sk_id: int) -> dict:
    ...
    if rows.empty:
        return {
            "sk_id_curr": int(sk_id),
            "n_previous": 0,
            "message": "Cliente novo no Home Credit.",
        }
